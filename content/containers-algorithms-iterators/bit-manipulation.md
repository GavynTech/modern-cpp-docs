...
Nearly every function on this page is a portable name for a circuit. CPUs have shipped dedicated bit hardware for decades: x86 exposes `popcnt`, `lzcnt`, `tzcnt`, `rol`, and `bswap`; Arm exposes `clz`, `rbit`, and `rev`. Before C++20 you reached that hardware through vendor intrinsics, or wrote a shift-and-mask recipe and hoped the optimizer recognized it; a `<bit>` function is a thin wrapper whose whole job is to name the instruction, so `std::popcount()` becomes one `popcnt` on x86 and `std::countl_zero()` one `clz` on Arm. Where the hardware lacks the instruction, the library substitutes a short branchless fallback (the classic portable popcount adds neighboring one-bit sums, then two-bit sums, then four-bit sums, a halving cascade that finishes a 32-bit word in about a dozen operations), so the answer is identical everywhere and only the speed changes. Because every function is also `constexpr`, the compiler can run the same logic itself during compilation, which is what lets a `static_assert` prove a bit-level fact before the program exists.

The counting and power-of-two families look like eight different questions, but underneath they are one question: where is the highest, or lowest, set bit? A power of two is exactly one set bit, so `has_single_bit(x)` is `popcount(x) == 1`. `bit_width(x)` is the type's width minus `countl_zero(x)`, turning a leading-zero count into "how many bits does this value occupy"; `bit_floor()` keeps only the highest set bit by shifting `1` left by `bit_width(x) - 1`, and `bit_ceil()` rounds upward by shifting `1` one past `bit_width(x - 1)`. The whole family therefore costs a leading-zero count and a shift, which is also exactly why `bit_ceil()` is undefined when the answer does not fit: the shift would need one more bit than the type has. The set-bit visiting idiom is the same insight read from the right end: subtracting one borrows through the trailing zeros, flipping them to ones and the lowest set bit to zero, so `x & (x - 1)` clears precisely that bit while `countr_zero()` (one `tzcnt`) names its index. It is ordinary arithmetic doing bit surgery.

The last sections earn their definedness through translation rather than run-time checks. `std::rotl()` reduces its count modulo the width (a negative count lands, by that same modular arithmetic, on the equivalent rotation the other way), and since the reduced operation matches what the rotate instruction already does, the wrapper collapses into a single `rol` or `ror`; the old `(x << n) | (x >> (w - n))` recipe was undefined at `n == 0` precisely because a shift, unlike a rotate, has no meaningful full-width case. `std::byteswap()` is specified as a pure permutation of the value's bytes, a shape compilers recognize and fold to `bswap` or `rev`, and the memcpy-then-swap pair from the endian sample fuses further into a single byte-reversing load on hardware that has one. `std::bit_cast()` is the extreme case: it is less an operation than a relabeling. The bytes already hold the answer, so the cast just tells the type system to read the same object representation as a new type, costing at most a register move and often nothing. In a constant expression there is no memory to relabel, so the compiler simulates the object representation bit for bit instead; that simulation is why pointers are refused there, because before the program runs an address has no bits to copy.
